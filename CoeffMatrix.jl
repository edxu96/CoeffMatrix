...
function updateRow(; mat_coeff = 0, add = 0, num_m, num_n, mat_para,
    ...
    if isa(mat_para, Number)  # If mat_para is a number or a vector, generate a m*n matrix for later use
        mat_para_cal = mat_para * ones(num_m, num_n)
    end
    ...
    elseif num_row_a < num_m  # if mat_para is a row vector, duplicate it to m-row matrix
        mat_para_cal = ones(num_m, num_n)
        for i = 1: num_m
            mat_para_cal[i, :] = mat_para[1, :]
        end
        mat_para = mat_para_new
    elseif num_col_a < num_n  # if mat_para is a strict column vector, duplicate it to n-column matrix
        mat_para_cal = ones(num_m, num_n)
        for j = 1: num_n
    ...
    for i = 1: length(vec_i_x)
        for j = 1: length(vec_j_x)
            vecRow_coeff_new[(vec_i_x[i] - 1) * num_n + vec_j_x[j]] = mat_para_cal[vec_i_a[i], vec_j_a[j]]
        end
    end
...
# (mat_coeff, mat_para_cal) = updateRow(num_m = 3, num_n = 12, mat_para = - hcat(mat_para[:,1]),
#     vec_i_x = [1 2 3], vec_j_x = [1])
# (mat_coeff, mat_para_cal) = updateRow(add = 1, mat_coeff = mat_coeff, num_m = 3, num_n = 12,
